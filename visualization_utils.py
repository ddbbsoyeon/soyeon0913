...
#사람 꼭짓점 좌표값 리스트 초기화
  ...
  for i , b in enumerate(boxes):
      if scores[i] >= 0.6:
          ...
          print("mid x",mid_x)
          print("mid y",mid_y)
          print("min x", boxes[i][1])
          print("min y", boxes[i][0])
          print("max x", boxes[i][3])
          print("max y", boxes[i][2])
          '''
  ...
  if 'warning' in e_list: # 해당 화면에 인식된 개체의 이름을 저장한 리스트에서 위험한 상황에 처한 객체가 있는지 확인합니다.
      ...
      print("확인된 쓰러진 사람이 없습니다. 위험상태: {}".format(is_warning))
      return image
...
def fence_check():
    ...
    for i, b in enumerate(fxy_list):
        for m in range(0, len(fxy_list[i]), 2):
            ...
            #print("fxy", fxy_list[i][m], fxy_list[i][m + 1])
            #print("num of people", len(fxy_list))
    if f_stat:
